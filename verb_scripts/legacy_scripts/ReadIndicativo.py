...
def read_html(verb):
    ...
    indexes = [
        (zuppa.find("PRESENTE")),
        (zuppa.find("IMPERFETTO")),
        zuppa.find("PASSATO REMOTO"),
        zuppa.find("TRAPASSATO PROSSIMO"),
        zuppa.find("PASSATO REMOTO"),
        zuppa.find("TRAPASSATO REMOTO"),
        zuppa.find("FUTURO SEMPLICE"),
        zuppa.find("FUTURO ANTERIORE"),
...
if __name__ == "__main__":
    ...
    for v in verb_list:
        ...
        for n in names:
            ...
            final_entry = v[0] + ";" + v[1] + ";"
            # final_items = list(filter((lambda it: any(ext in it for ext in ["io", "tu", "lei", "lui", "noi", "voi", "loro"])), final_items))
    ...
    with open("indicativo.json", "w") as f:
        json.dump(verbs, f)
